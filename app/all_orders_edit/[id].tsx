...
const EditOrder: React.FC = () => {
  ...
  const { getFormDataById, updateFormData } = useAppwriteFormData();
...
useEffect(() => {
  const loadData = async () => {
    ...
    try {
      const data = await getFormDataById(id as string) as OrderData;
...
const handleSubmit = async () => {
    ...
    const updatedData: Partial<FormData> = {
      ...
      deviceKYC: {
        ...deviceKYCDetails,
        warrantyExpiryDate: deviceKYCDetails.warrantyExpiryDate ? new Date(deviceKYCDetails.warrantyExpiryDate).toISOString() : null,
      },
    };
